...
**A chunk can carry a `secondary_emotion`, added later.** A paragraph can genuinely express two emotions at once ("proud but anxious about what's next"), and forcing a single dominant label discarded that. The model only fills it in when a second emotion is clearly and separately expressed, never to fill the field — it's nullable, and stays null for most chunks. `secondary_emotion_confidence` mirrors `emotion_confidence`: display-only, shown in `/debug/chunks` for hand-checking, never used to gate or weight search results (nothing in this pipeline gates on confidence for the primary emotion either — `filter_chunks` only ever checks equality). `filter_chunks` matches a requested emotion against either column, so a chunk is findable by either of its two emotions equally.
...
- **Layer 1 (no LLM call).** `supabase/functions/_shared/emotionSynonyms.ts` matches the question against a synonym table seeded from the closed emotion vocabulary (phase 2) plus common synonyms ("sad"/"down"/"blue" → `sadness`). `supabase/functions/_shared/vocabMatch.ts` matches the question against topic/entity strings that already exist in the user's own `chunk_metadata`, via a new `match_known_vocab` Postgres function (`supabase/migrations/0007_rag_phase7_query_extraction.sql`) built on `pg_trgm`'s `word_similarity` — the right primitive for "does this known short term appear somewhere in this longer question", as opposed to whole-string similarity.
  - **Fixed in `supabase/migrations/0008_fix_vocab_match_short_terms.sql`:** `word_similarity` alone let short terms produce false positives — "mom" scored 0.75 against "sad moment" (it's a literal 3-character prefix of "moment"), well above the match threshold. No threshold value fixes this; short strings just don't carry enough trigram signal to separate a real match from a coincidental one. Terms of 5+ characters still fuzzy-match via `word_similarity`; shorter terms now require an exact whole-word match instead (same normalize-and-pad technique `matchEmotionKeywords` uses). Known tradeoff: this also stops fuzzy-catching some short-term variants (e.g. "job" no longer matches "jobs") — accepted in favor of not matching on coincidence.
  - `matchEmotionKeywords` returns every distinct emotion whose keyword appears in the question, ordered by earliest occurrence in the text — not by the synonym table's definition order, which has no relationship to the question itself. An earlier version returned only the first dictionary match, so e.g. "proud but anxious" silently returned whichever of the two happened to be defined first in `emotionSynonyms.ts`, regardless of what the question actually said.
- **Layer 2 (LLM fallback).** `supabase/functions/_shared/queryExtraction.ts` calls `gpt-4o-mini`, but only asks for whatever Layer 1 left unresolved — if Layer 1 resolved everything, this never runs. `emotion` and `topics` are both constrained to a closed list (the 15-label enum for emotion, the user's own existing topic vocabulary for topics) and can return an empty array rather than force a mapping; `entities` is constrained the same way, against the user's own existing entity vocabulary, for the identical reason topics is — open generation previously let the model echo the question back as a value that matched zero real chunks (verified: "times I felt weird about my job" resolves `topics: ['job']` via Layer 1, leaves `emotion: []` — "weird" doesn't map cleanly to any label).
