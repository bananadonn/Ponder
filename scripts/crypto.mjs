...
//
// Keep the primitives section of this file byte-for-byte identical (modulo
// TS type annotations, which this plain-JS Node file omits) across:
...
// identically in browsers, Deno, and Node >=19 (this script targets Node,
// same runtime as the other scripts/*.mjs files).
...
export async function importAesKey(rawKey, extractable = false) {
  return crypto.subtle.importKey('raw', rawKey, ALGO, extractable, ['encrypt', 'decrypt'])
}
...
export async function decryptOrPassthrough(key, value) {
...
export async function encryptBytes(key, plaintext) {
  const iv = crypto.getRandomValues(new Uint8Array(IV_BYTES))
  const ciphertext = new Uint8Array(await crypto.subtle.encrypt({ name: ALGO, iv }, key, plaintext))
...
// --- Server-only: per-user DEK lookup/creation (mirrors
// supabase/functions/_shared/crypto.ts -- not present in src/lib/crypto.ts,
// the browser never reads/writes user_keys directly). ---

// Race-safe create-or-fetch: see the Deno copy of this function for the
// full rationale. This script also runs single-threaded and sequentially
// per user in practice, but keeps the same conflict-safe logic so it's
// safe to run concurrently with the app's own derive-key calls.
export async function getUserDekBytes(admin, masterKey, userId) {
...
export async function getUserDek(admin, masterKey, userId) {
